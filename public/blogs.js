...
window.addEventListener("load", () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    // Store the posts
    let postsArray = [];
    const storedPosts = yield JSON.parse(localStorage.getItem("posts") || "[]");
    ...
    yield localStorage.setItem("posts", JSON.stringify(postsArray));
    const localPosts = yield JSON.parse(localStorage.getItem("posts") || "[]");
    const users = yield JSON.parse(localStorage.getItem("users") || "[]");
    // create Elements for posts
    let postsElement = "";
    for (let i = 0; i < posts.length - 1; i++) {
        const preview = (_a = posts[i]) === null || _a === void 0 ? void 0 : _a.body.substr(0, 100);
        const user = users.find((user) => user.id === posts[i].userId);
        postsElement += `
